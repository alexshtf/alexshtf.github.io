...
In the recent [interview](https://www.youtube.com/watch?v=aR20FWCCjAs) with Ilya Sutskever there was one interesting insight - perhaps we need neurons to do more compute than they do now. And it immediately rang a bell, and returned me to my Ph.D days - what would be this "more compute" that is useful? Well, maybe a neuron can solve a small optimization problem! 

So in this post we shall explore an idea that most optimization researchers are familiar with - the eigenvalues of a matrix are solutions to optimization problems that, on the one hand, can model extremely complicated functions, but on the other hand, we have a lot of machinery to reason about them and understand their properties. So what can one such neuron do? Turns out quite a lot! This is exactly what we shall explore in this post. The notebook for reproducing all the results is [here](https://github.com/alexshtf/alexshtf.github.io/blob/master/assets/specrum_power_intro.ipynb). Feel free to deploy it to Colab and play ith it.
...
So what do the functions look like? Let's take two $$3 \times 3$$ matrices $$\mathbf{A}$$ and $$\mathbf{B}$$ and plot the three functions corresponding the three eigenvalues:
...
Indeed appears a concave function with some non-trivial shape. 
...
def make_trainset(n_train: int = 100, train_noise: float = 0.2):
    np.random.seed(42)
...
Alright! Let's train our smallest eigenvalue model! Below is a pretty standard PyTorch training loop with several epochs:

```python
from torch.utils.data import DataLoader
import math

def train_model(dim: int, n_epochs: int = 500, batch_size = 5):
    lr = 1e-3
    print_every = n_epochs // 10
    dl = DataLoader(ds, batch_size=batch_size, shuffle=True)

    model = BivariateSpectral(dim, 0)
...
Alright. Let's try training a model with $$3 \times 3$$ matrices, and plot it:

```python
model = train_model(3)
with torch.no_grad():
    Z = model(as_tensor(X), as_tensor(Y)).numpy()
plot_bivariate(X, Y, Z)
```

The output:

```
Epoch 50, train RMSE:  0.2774
Epoch 100, train RMSE:  0.1244
Epoch 150, train RMSE:  0.1016
Epoch 200, train RMSE:  0.0945
Epoch 250, train RMSE:  0.0900
Epoch 300, train RMSE:  0.0879
Epoch 350, train RMSE:  0.0867
Epoch 400, train RMSE:  0.0856
Epoch 450, train RMSE:  0.0855
Epoch 500, train RMSE:  0.0850
```
...
Now let's try something interesting - matrices of size $$20 \times 20$$. Note, that even _one_ such matrix has more entries than the size of the training set. So such a model is heavily over-parametrized. To make sure we're converging to the smallest loss we can, let's also crank up the number of epochs:

```python
model = train_model(20, n_epochs=2000)
with torch.no_grad():
    Z = model(as_tensor(X), as_tensor(Y)).numpy()
plot_bivariate(X, Y, Z)
```
...
```
Epoch 200, train RMSE:  0.3814
Epoch 400, train RMSE:  0.0762
Epoch 600, train RMSE:  0.0744
Epoch 800, train RMSE:  0.0765
Epoch 1000, train RMSE:  0.0752
Epoch 1200, train RMSE:  0.0741
Epoch 1400, train RMSE:  0.0730
Epoch 1600, train RMSE:  0.0723
Epoch 1800, train RMSE:  0.0734
Epoch 2000, train RMSE:  0.0737
```

![pow_spec_concave_bi_model_20]({{"assets/pow_spec_concave_bi_model_20.png" | absolute_url}})

Whoa! This is interesting! Even though the model appears over-parametrized - it does not memorize the training set! Why? Well, because it is _concave by design_, and there appears to be no concave function that can achieve a zero training loss. Also, we see that the model is not "crazy" - its shape slightly resembles the true one, even if it's a bit skewed.  Does it improve with more data? Well, it turns out it does! Here is what happens if we use an over-parametrized model with a 50 times larger training set:

```python
x_train, y_train, z_train = make_trainset(5000) # 50 times larget training set.
ds = TensorDataset(
    as_tensor(x_train), as_tensor(y_train), as_tensor(z_train)
)

model = train_model(100, n_epochs=20)
with torch.no_grad():
    Z = model(as_tensor(X), as_tensor(Y)).numpy()
plot_bivariate(X, Y, Z)
```
...
```python
Epoch 2, train RMSE:  10.9518
Epoch 4, train RMSE:  5.8636
Epoch 6, train RMSE:  2.6577
Epoch 8, train RMSE:  0.7886
Epoch 10, train RMSE:  0.1728
Epoch 12, train RMSE:  0.1001
Epoch 14, train RMSE:  0.1002
Epoch 16, train RMSE:  0.1000
Epoch 18, train RMSE:  0.0984
Epoch 20, train RMSE:  0.0966
```

![pow_spec_concave_bi_model_100_5000]({{"assets/pow_spec_concave_bi_model_100_5000.png" | absolute_url}})

Turns out we get a very nice fit. And as expected - the training loss does _not_ go to zero, despite the apparently huge number of parameters, because there is no concave function that can achieve a zero training loss.  Turns out our knowledge of the function being concave is a **very** strong inductive bias! 

Now it would be a good time to do a recap, because we have much more things to explore.
...
- What about a general function? How do eigenvalue functions compare to just plain old ReLU MLPs?
